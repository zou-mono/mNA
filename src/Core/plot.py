...
from Core import utils_graph, graph, simplification, settings, projection
...
def plot_graph(
    ...
    log.info("Begin plotting the graph...")
    if ax is None:
    ...
    log.info("Finished plotting the graph")
    return fig, ax
...
def _save_and_show(fig, ax, save=False, show=True, close=True, filepath=None, dpi=300):
    ...
    if save:
        ...
        log.info(f"Saved figure to disk at {filepath}")
